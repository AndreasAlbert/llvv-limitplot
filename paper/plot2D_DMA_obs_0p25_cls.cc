...
void plot2D_DMA_obs_0p25_cls(TString myfolder = "", TString tag="", bool savePlots=true)
{
    ...
    for(size_t nmx=0; nmx<dm_masses.size(); nmx++) {
      ...
      if(dm_masses[nmx]>300.) continue;
        ...
        for(int nmv=0; nmv<sizemv; nmv++) {
            ...
            TString folder = myfolder+"/"+tag+"_"+str_mx+"_"+str_mv+"_0p25";
    ...
    // Relic Density
    TGraph * h_relic = new TGraph("relic_a_25.txt");
    ...
    TGraph * h_relic2 = new TGraph("list_scan_A_g25_MD_xxd_A_gq0.25_gdm1.0_contour_10.txt");
    ...
    addText(0.38,0.48,0.5,0.6,"#Omega_{c} #times h^{2} = 0.12",getRelicColor(), 60, 0.04);
    ...
    //~ leg->Draw();
    if(savePlots){
        canv->SaveAs("Limit_Mx_"+tag+"_obs_0p25_cls.png");
        canv->SaveAs("Limit_Mx_"+tag+"_obs_0p25_cls.pdf");
    }
    delete canv;
